...
export default function History() {
  ...
  useEffect(() => {
    if (!player) return;
    getPlayerHistory(player.telegramId)
      .then(data => { setGames(data.games ?? []); setWithdrawals(data.withdrawals ?? []); })
      .finally(() => setLoading(false));
  }, [player]);

  const visibleGames = useMemo(
    () => (filter === "all" ? games : games.filter(row => row.game === filter)),
    [games, filter],
  );
  const gameFilters = useMemo(
    () => [...new Set(games.map(row => row.game).filter((g): g is string => Boolean(g)))],
    [games],
  );
  const gameName = (key: string | null) => (key && GAME_NAMES[key]?.[lang]) || u("gameCol");
  const date = (value: string) =>
    new Intl.DateTimeFormat(lang === "ru" ? "ru-RU" : lang === "en" ? "en-US" : "uz-UZ", {
    ...
    <div className="min-h-screen" style={{ background: pageBg(theme, GAME_BG.home) }}>
      ...
      <main className="px-4 pb-10">
        <div className="grid grid-cols-2 gap-2 mb-4 p-1 rounded-2xl"
          ...
          {([["games", `🎮 ${u("tabGames")}`], ["withdrawals", `💸 ${u("tabWithdrawals")}`]] as const).map(([key, label]) => (
            <button key={key} onClick={() => setTab(key)}
              className="py-3 rounded-xl text-sm font-black transition-transform active:scale-95"
              style={{ background: tab === key ? GOLD.grad : "transparent", color: tab === key ? "#211604" : ts.textSub }}>
              {label}
            </button>
          ))}
        </div>

        {tab === "games" && gameFilters.length > 0 && (
          <div className="flex gap-2 overflow-x-auto pb-3 mb-1">
            {["all", ...gameFilters].map(game => (
              <button key={game} onClick={() => setFilter(game)}
                className="shrink-0 px-3 py-2 rounded-xl text-xs font-bold"
                style={{
                  background: filter === game ? GOLD.soft : ts.input,
                  border: `1px solid ${filter === game ? GOLD.border : ts.inputBorder}`,
                  color: filter === game ? GOLD.light : ts.textSub,
                }}>
                {game === "all" ? u("all") : gameName(game)}
              </button>
            ))}
          </div>
        )}
          ...
          {!loading && tab === "games" && visibleGames.map(row => (
            <div key={row.id} className="flex items-center justify-between p-4 rounded-2xl pop-in"
              ...
              <div className="text-right">
                ...
                <p className="text-xs" style={{ color: ts.textSub }}>
                  {row.type === "win" ? u("win") : u("lose")}
                </p>
              </div>
            </div>
          ))}
            ...
            <div key={row.id} className="flex items-center justify-between p-4 rounded-2xl pop-in"
              ...
              <div className="text-right">
                ...
                <p className="text-xs font-bold"
                  ...
                  {row.status === "approved" ? u("stPaid") : row.status === "rejected" ? u("stRejected") : u("stPending")}
                </p>
              </div>
            </div>
          ))}

          {!loading && ((tab === "games" && visibleGames.length === 0) || (tab === "withdrawals" && withdrawals.length === 0)) && (
            <p className="text-center py-12" style={{ color: ts.textSub }}>{u("noHistory")}</p>
          )}
        </div>
      </main>
    </div>
