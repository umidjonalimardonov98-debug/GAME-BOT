...
export default function PvpPoker() {
  ...
  useEffect(() => {
    api("/pvp-poker/config").then(setCfg).catch(() => {});
  ...
  const join = async (s: number) => {
    ...
    setErr(""); setStake(s);
    try {
      const r = await api("/pvp-poker/queue", { telegramId: player.telegramId, name: player.firstName ?? "O'yinchi", stake: s });
  ...
  const cancel = async () => {
    if (!player) return;
    if (poll.current) clearInterval(poll.current);
    await api("/pvp-poker/cancel", { telegramId: player.telegramId }).catch(() => {});
  ...
  const startPoll = (rid: string) => {
    ...
    poll.current = setInterval(async () => {
      if (!player) return;
      try {
        const v: View = await api(`/pvp-poker/state?telegramId=${player.telegramId}&roomId=${rid}`);
        setView(v);
        if (v.winner) {
          setStatus("done");
          if (poll.current) clearInterval(poll.current);
  ...
  const move = async (action: string) => {
    ...
    setErr("");
    try {
      await api("/pvp-poker/move", { telegramId: player.telegramId, roomId, action });
      sfx.select();
  ...
  const forfeit = async () => {
    if (!player || !roomId) return;
    await api("/pvp-poker/forfeit", { telegramId: player.telegramId, roomId }).catch(() => {});
  };
  ...
  if (status === "idle" || status === "waiting") {
    return (
      <div className="min-h-screen flex flex-col overflow-x-hidden" style={{ background: BG }}>
        <GameHeader icon="cardback" title=" LIVE PVP · POKER" subtitle="Texas Hold'em 1x1 · haqiqiy raqib" />
        <div className="flex-1 px-4 pb-8 flex flex-col gap-4">
          ...
          <div className="rounded-3xl p-4 flex items-center justify-around"
            ...
            <div className="text-center">
              <p className="font-black text-lg" style={{ color: "#38bdf8" }}>1x1</p>
              <p style={{ fontSize: 10, color: ts.textSub }}>HEADS-UP</p>
            </div>
          </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {(cfg?.stakes ?? [5000, 10000, 25000, 50000, 100000]).map((s) => (
                  <button key={s} onClick={() => join(s)}
                    ...
                    style={{
                      background: "linear-gradient(160deg,#6b1a1a,#3b0a0a)",
                      border: "1px solid rgba(247,201,72,.35)", color: "#fff",
                    }}>
                    {s.toLocaleString()}
                    <div style={{ fontSize: 10, color: "#f7c948" }}>bank {Math.floor(s * 2 * 0.92).toLocaleString()}</div>
                  </button>
                ))}
              </div>
            </>
          ...
          <div className="rounded-3xl p-4" style={{ background: ts.card, border: `1px solid ${ts.cardBorder}` }}>
            ...
            <ul className="space-y-1" style={{ fontSize: 11, color: ts.textSub, lineHeight: 1.5 }}>
              <li>• Har ikkalasiga 2 tadan yopiq karta beriladi, avtomatik blind qo'yiladi.</li>
              <li>• 1-pog'ona: CHECK, CALL yoki RAISE bilan tikish.</li>
              <li>• Keyin 5 ta umumiy karta bir vaqtda ochiladi.</li>
              <li>• 2-pog'ona tikishdan so'ng eng kuchli qo'l g'olib bo'ladi.</li>
              <li>• FOLD bossangiz — raqib darhol yutadi.</li>
              <li>• 30 soniya harakatsizlik — avtomatik CHECK yoki FOLD.</li>
              <li>• Yutuq: umumiy bank, uy ulushi 8%.</li>
            </ul>
          </div>
  ...
  return (
    <div className="min-h-screen flex flex-col overflow-x-hidden" style={{ background: BG }}>
      <GameHeader icon="cardback" title=" POKER PVP" subtitle={v ? `${v.stake.toLocaleString()} · ${v.foeName}` : "..."} />
      <div className="flex-1 px-3 pb-6 flex flex-col gap-3">
        ...
        {v && (
          <>
            ...
            <div className="rounded-3xl p-3 min-h-[150px] flex flex-col items-center justify-center gap-2"
              style={{
                background: "radial-gradient(circle at 50% 20%, #6b1a1a, #1f0505 70%)",
                border: `1px solid ${ts.cardBorder}`,
                boxShadow: "inset 0 0 60px rgba(0,0,0,.6)",
              }}>
              <div className="flex items-center justify-between w-full" style={{ fontSize: 11, color: "rgba(255,255,255,.75)" }}>
                <span>Bank: <b style={{ color: "#f7c948" }}>{v.pot.toLocaleString()}</b></span>
                <span>{v.street === "preflop" ? "PREFLOP" : v.street === "board" ? "BOARD" : "SHOWDOWN"}</span>
              </div>
            ...
            {!v.winner ? (
              <div className="grid grid-cols-3 gap-2">
                <button disabled={!v.myTurn} onClick={() => move(v.myBet === v.foeBet ? "check" : "call")}
                  className="rounded-xl py-3 font-black active:scale-95"
                  style={{ fontSize: 12, background: "rgba(34,197,94,.2)", color: "#4ade80", opacity: v.myTurn ? 1 : 0.45 }}>
                  {v.myBet === v.foeBet ? "CHECK" : "CALL"}
                </button>
                <button disabled={!v.myTurn} onClick={() => move("raise")}
                  className="rounded-xl py-3 font-black active:scale-95"
                  style={{ fontSize: 12, background: "linear-gradient(160deg,#f7e59b,#d4af37)", color: "#1a1200", opacity: v.myTurn ? 1 : 0.45 }}>
                  RAISE
                </button>
                <button disabled={!v.myTurn} onClick={() => move("fold")}
                  className="rounded-xl py-3 font-black active:scale-95"
                  style={{ fontSize: 12, background: "rgba(239,68,68,.2)", color: "#f87171", opacity: v.myTurn ? 1 : 0.45 }}>
                  FOLD
                </button>
              </div>
            ) : (
