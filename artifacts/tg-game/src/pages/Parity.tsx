...
export default function Parity() {
  ...
  const BET_OPTIONS: { type: BetType; label: string; sub: string; mult: number; emoji: string; color: string; bg: string; border: string; glow: string }[] = [
    { type: "even",  label: t.even,  sub: "0,2,4,6,8", mult: 2, emoji: "2️⃣", color: "#60a5fa", bg: "linear-gradient(145deg,#1e3a5f,#1e40af)", border: "#3b82f666", glow: "#3b82f633" },
    { type: "odd",   label: t.odd,   sub: "1,3,5,7,9", mult: 2, emoji: "1️⃣", color: "#f87171", bg: "linear-gradient(145deg,#7f1d1d,#dc2626)", border: "#f87171aa", glow: "#f8717133" },
    { type: "small", label: t.small, sub: "0–4",        mult: 2, emoji: "⬇️", color: "#4ade80", bg: "linear-gradient(145deg,#064e3b,#059669)", border: "#10b98166", glow: "#10b98133" },
    { type: "big",   label: t.big,   sub: "5–9",        mult: 2, emoji: "⬆️", color: "#fbbf24", bg: "linear-gradient(145deg,#78350f,#d97706)", border: "#f59e0b66", glow: "#f59e0b33" },
  ];
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "linear-gradient(160deg, #0a0a1a 0%, #0f0a2a 50%, #0a0a1a 100%)" }}>
      ...
      <div className="flex items-center gap-3 px-4 pt-5 pb-3">
        ...
        <div>
          <p className="text-white font-black text-lg">🔢 {t.parityTitle}</p>
          <p className="text-xs" style={{ color: "rgba(255,255,255,0.45)" }}>{t.even}/{t.odd} x2</p>
        </div>
      ...
      <div className="flex-1 px-4 pb-6 flex flex-col gap-4">
        ...
        {/* Number display — 3D */}
        <div className="rounded-3xl p-6 flex flex-col items-center gap-3 mt-1 relative overflow-hidden"
          style={{
            background: "linear-gradient(145deg, #1a1a3a, #0f0f2a)",
            border: "2px solid rgba(255,255,255,0.08)",
            boxShadow: "0 10px 0 rgba(0,0,0,0.5), 0 14px 40px rgba(0,0,0,0.4), inset 0 2px 0 rgba(255,255,255,0.06)",
          }}>
          ...
          <p className="text-xs font-bold tracking-widest" style={{ color: "rgba(255,255,255,0.3)" }}>{t.number}</p>

          <div className="w-32 h-32 rounded-3xl flex items-center justify-center relative"
            style={{
              background: displayNum !== null
                ? (isRed(displayNum) ? "linear-gradient(145deg, #7f1d1d, #dc2626)" : "linear-gradient(145deg, #1e3a5f, #2563eb)")
                : "rgba(255,255,255,0.05)",
              border: displayNum !== null
                ? (isRed(displayNum) ? "3px solid #f87171aa" : "3px solid #60a5faaa")
                : "2px solid rgba(255,255,255,0.1)",
              boxShadow: displayNum !== null
                ? (isRed(displayNum) ? "0 8px 0 #4a0505, 0 10px 30px #dc262655" : "0 8px 0 #0a1a40, 0 10px 30px #2563eb55")
                : "0 4px 0 rgba(0,0,0,0.5)",
              transition: "all 0.15s",
            }}>
            <div className="absolute inset-0 rounded-3xl pointer-events-none" style={{ background: "linear-gradient(135deg, rgba(255,255,255,0.2) 0%, transparent 50%)" }} />
            <span className="font-black relative" style={{ fontSize: 60, color: "white", textShadow: "0 4px 12px rgba(0,0,0,0.6)" }}>
          ...
          {gameState === "result" && (
            <div className="text-center">
              <p className="font-black text-xl" style={{ color: won ? "#4ade80" : "#f87171", textShadow: won ? "0 0 20px #4ade8088" : "none" }}>
          ...
          {BET_OPTIONS.map(opt => (
            <button key={opt.type} onClick={() => setBetType(opt.type)}
              ...
              style={{
                ...
                boxShadow: betType === opt.type
                  ? `0 5px 0 rgba(0,0,0,0.4), 0 6px 20px ${opt.glow}, inset 0 1px 0 rgba(255,255,255,0.15)`
              ...
              <p className="font-black text-base relative" style={{ color: betType === opt.type ? opt.color : "rgba(255,255,255,0.7)" }}>
                {opt.emoji} {opt.label}
              </p>
              <p className="text-xs mt-0.5 relative" style={{ color: "rgba(255,255,255,0.4)" }}>{opt.sub}</p>
              <p className="text-xs font-black mt-1 relative" style={{ color: betType === opt.type ? opt.color : "rgba(255,255,255,0.3)" }}>x{opt.mult}</p>
            </button>
          ))}
        </div>
        ...
        <div className="rounded-2xl p-4" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
          ...
          {selectedOpt && (
            <p className="text-xs mt-2 text-center" style={{ color: "rgba(255,255,255,0.3)" }}>
              {t.potential}: <b style={{ color: selectedOpt.color }}>{(activeBet * selectedOpt.mult).toLocaleString()} UZS</b>
            </p>
          )}
        </div>
        ...
        <button onClick={play}
          ...
          style={{
            background: betType
              ? (BET_OPTIONS.find(b => b.type === betType)?.bg ?? "linear-gradient(145deg, #4c1d95, #6d28d9)")
              : "rgba(255,255,255,0.08)",
            color: "white",
            boxShadow: betType ? `0 6px 0 rgba(0,0,0,0.4), 0 8px 24px ${selectedOpt?.glow ?? "transparent"}` : "none",
          }}>
