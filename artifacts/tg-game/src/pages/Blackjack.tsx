...
function CardUI({ card, hidden }: { card: Card; hidden?: boolean }) {
  ...
  if (hidden) return (
    <div className="flex items-center justify-center rounded-xl font-black relative overflow-hidden"
      style={{ width: 58, height: 84,
        background: "linear-gradient(145deg, #312e81, #4338ca)",
        border: "2px solid #6366f1",
        boxShadow: "0 4px 0 #1e1b4b, 0 6px 16px rgba(0,0,0,0.6), inset 0 1px 0 rgba(255,255,255,0.15)" }}>
      <div className="absolute inset-0" style={{ background: "linear-gradient(135deg, rgba(255,255,255,0.1) 0%, transparent 50%)" }} />
      <span style={{ fontSize: 28, opacity: 0.4 }}>🂠</span>
    </div>
  );
  return (
    <div className="flex flex-col justify-between p-1.5 rounded-xl font-black relative overflow-hidden"
      style={{ width: 58, height: 84,
        background: "linear-gradient(145deg, #ffffff, #f1f5f9)",
        border: "2px solid rgba(255,255,255,0.5)",
        boxShadow: "0 4px 0 rgba(0,0,0,0.3), 0 6px 16px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.9)" }}>
      <div className="absolute inset-0" style={{ background: "linear-gradient(135deg, rgba(255,255,255,0.4) 0%, transparent 50%)" }} />
      <div className="relative" style={{ fontSize: 13, color: red ? "#dc2626" : "#1e1b4b", lineHeight: 1 }}>
        {card.value}<br />{card.suit}
      </div>
      <div className="relative" style={{ fontSize: 13, color: red ? "#dc2626" : "#1e1b4b", lineHeight: 1, textAlign: "right", transform: "rotate(180deg)" }}>
        {card.value}<br />{card.suit}
      </div>
    </div>
  );
...
export default function Blackjack() {
  ...
  const RESULT_CONFIG: Record<NonNullable<Result>, { label: string; color: string; bg: string }> = {
    blackjack: { label: t.bjBlackjack, color: "#fbbf24", bg: "linear-gradient(145deg, #78350f, #d97706)" },
    win:       { label: t.bjWin,       color: "#4ade80", bg: "linear-gradient(145deg, #064e3b, #059669)" },
    push:      { label: t.bjPush,      color: "#93c5fd", bg: "linear-gradient(145deg, #1e3a5f, #1e40af)" },
    bust:      { label: t.bjBust,      color: "#f87171", bg: "linear-gradient(145deg, #7f1d1d, #dc2626)" },
    lose:      { label: t.bjLose,      color: "#f87171", bg: "linear-gradient(145deg, #7f1d1d, #dc2626)" },
  };
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "linear-gradient(160deg, #0f1a0f 0%, #0a2010 50%, #0f1a0f 100%)" }}>
      ...
      <div className="flex items-center gap-3 px-4 pt-5 pb-3">
        ...
        <div>
          <p className="text-white font-black text-lg">🃏 {t.blackjackTitle}</p>
      ...
      <div className="flex-1 px-4 pb-6 flex flex-col gap-3">
        ...
        {/* Dealer */}
        <div className="rounded-2xl p-4" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", boxShadow: "0 4px 16px rgba(0,0,0,0.3)" }}>
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs font-bold tracking-widest" style={{ color: "rgba(255,255,255,0.5)" }}>🤵 {t.dealer}</p>
            {gameState !== "idle" && (
              <span className="text-sm font-black text-white px-2 py-0.5 rounded-lg" style={{ background: "rgba(255,255,255,0.1)" }}>
                {dealerRevealed ? dealerTotal : "?"}
              </span>
            )}
          </div>
          <div className="flex gap-2 flex-wrap min-h-[84px] items-center">
            {dealerCards.map((c, i) => <CardUI key={i} card={c} hidden={i === 1 && !dealerRevealed} />)}
            {gameState === "idle" && <span style={{ color: "rgba(255,255,255,0.2)", fontSize: 13 }}>{t.cardsLoading}</span>}
          </div>
        </div>
        ...
        {result && (
          <div className="rounded-2xl p-4 text-center relative overflow-hidden"
            style={{ background: RESULT_CONFIG[result].bg, boxShadow: "0 6px 0 rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.15)" }}>
            ...
            <p className="relative font-black text-2xl" style={{ color: RESULT_CONFIG[result].color, textShadow: "0 2px 8px rgba(0,0,0,0.5)" }}>{RESULT_CONFIG[result].label}</p>
        ...
        <div className="rounded-2xl p-4" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", boxShadow: "0 4px 16px rgba(0,0,0,0.3)" }}>
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs font-bold tracking-widest" style={{ color: "rgba(255,255,255,0.5)" }}>👤 {t.yourHand}</p>
            {gameState !== "idle" && (
              <span className="text-sm font-black px-2 py-0.5 rounded-lg"
                style={{ background: "rgba(255,255,255,0.1)", color: playerTotal > 21 ? "#f87171" : "white" }}>
                {playerTotal}
              </span>
            )}
          </div>
        ...
        {(gameState === "idle" || gameState === "result") && (
          <div className="rounded-2xl p-4" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
            ...
            <p className="text-xs mt-2 text-center" style={{ color: "rgba(255,255,255,0.3)" }}>{t.bjPayout}</p>
          </div>
        )}
        ...
        <div className="flex flex-col gap-3 mt-auto">
          {(gameState === "idle" || gameState === "result") ? (
            <button onClick={deal} disabled={!player || player.balance < activeBet}
              ...
              style={{ background: "linear-gradient(145deg, #166534, #16a34a)", boxShadow: "0 6px 0 #0d3d1e, 0 8px 24px #16a34a44", color: "white" }}>
              🃏 {gameState === "result" ? t.bjPlayAgain : t.bjDeal}
            </button>
          ) : gameState === "playing" ? (
            <div className="grid grid-cols-2 gap-3">
              <button onClick={hit}
                className="py-4 rounded-2xl font-black text-base active:scale-95 transition-all"
                style={{ background: "linear-gradient(145deg, #1e40af, #2563eb)", boxShadow: "0 5px 0 #1e3a8a, 0 6px 16px #2563eb44", color: "white" }}>
                ➕ {t.bjHit}
              </button>
              <button onClick={stand}
                className="py-4 rounded-2xl font-black text-base active:scale-95 transition-all"
                style={{ background: "linear-gradient(145deg, #92400e, #d97706)", boxShadow: "0 5px 0 #451a03, 0 6px 16px #d9770644", color: "white" }}>
                ✋ {t.bjStand}
              </button>
            </div>
          ) : null}
        ...
        {saving && <p className="text-center text-xs" style={{ color: "rgba(255,255,255,0.4)" }}>{t.saving}</p>}
      </div>
    </div>
  );
}
