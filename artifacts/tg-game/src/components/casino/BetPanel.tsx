...
export default function BetPanel({ value, onChange, onQuick, disabled }: Props) {
  ...
  const { t } = useLang();
  return (
    <div className="w-full rounded-xl p-3" style={{ background: ts.card, border: `1px solid ${ts.cardBorder}` }}>
      ...
      <div className="grid grid-cols-4 gap-2 mb-3">
        {["MIN", "1/2", "X2", "MAX"].map(a => (
          <button key={a} disabled={disabled} onClick={() => onQuick(a)}
            className="py-2 rounded-lg text-xs font-bold active:scale-95 transition-transform disabled:opacity-40"
      ...
      <input type="number" value={value} disabled={disabled} onChange={e => onChange(e.target.value)}
        className="w-full rounded-lg px-4 py-3 font-black text-center text-lg outline-none disabled:opacity-60"
